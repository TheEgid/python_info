...
def save_model_locally(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Загружаем во временную директорию для безопасности
            ...
            logger.error(f"❌ Непредвиденная ошибка при попытке {attempt + 1}: {type(e).__name__}")
...
def _check_disk_space(path: Path, min_space_gb: float = 2.0) -> None:
    ...
    try:
        ...
        if free_gb < min_space_gb:
            logger.warning(f"⚠ Недостаточно места! Требуется минимум {min_space_gb} GB, доступно {free_gb:.2f} GB")
    except Exception as e:
...
def get_frida_embeddings(
    ...
    try:
        # Определяем источник модели и загружаем её
        if local_model_dir:
...
def _get_or_download_model(
    ...
    Args:
        ...
        local_model_dir (str): Директория для сохранения.
        force_redownload (bool): Принудительная перезагрузка.
