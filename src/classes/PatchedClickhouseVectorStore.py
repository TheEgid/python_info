...
class PatchedClickhouseVectorStore(ClickhouseVectorStore):
    """Patched ClickHouse vector store with enhanced error handling and normalization."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Initialized ClickHouse vector store with metric: {metric}, dim: {embedding_dim}"
        )
    ...
    def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:  # noqa: ANN401
        """Выполняет поиск по векторному хранилищу ClickHouse."""
        try:
            # Проверяем что клиент существует
            if self._client is None:
                logger.error("ClickHouse client is not initialized")
            ...
            logger.info(
                f"Performing vector search with top_k={top_k}, metric={self._metric}"
            )

            # Используем ClickHouse для поиска
            # В ClickHouse поиск обычно выполняется через SQL запросы с функциями расстояния
            results = self._perform_clickhouse_search(query_embedding, top_k)
            ...
            logger.exception(f"Error in PatchedClickhouseVectorStore.query: {e}")
            return VectorStoreQueryResult(nodes=[], similarities=[], ids=[])

    def _perform_clickhouse_search(
        self, query_embedding: np.ndarray, top_k: int
    ) -> List[dict]:
        """Выполняет поиск в ClickHouse используя векторный запрос."""
        try:
            # Конвертируем embedding в список для ClickHouse
            embedding_list = query_embedding.tolist()

            # Строим SQL запрос для векторного поиска
            # Предполагаем, что у нас есть таблица с векторами и метаданными
            if self._metric == "cosine":
                # Для косинусного расстояния используем cosineDistance
                distance_func = "cosineDistance"
            else:
                # Для L2 используем L2Distance
            ...
            SELECT
                ...
                metadata,
                embedding,
                {distance_func}(embedding, %(embedding)s) as distance
            FROM {self.table_name}
            ORDER BY distance ASC
            LIMIT %(top_k)s
            """

            params = {"embedding": embedding_list, "top_k": top_k}

            # Выполняем запрос
            results = self._client.execute(query_sql, params)

            # Конвертируем результаты в список словарей
            columns = ["id", "text", "metadata", "embedding", "distance"]
            return [dict(zip(columns, row)) for row in results]

        except Exception as e:
            logger.error(f"Error performing ClickHouse search: {e}")
            return []
    ...
    def _create_text_node(
        self, row: pd.Series, metadata: dict, text_content: str, idx: int
    ) -> TextNode:
    ...
    def _extract_distance(self, row: pd.Series) -> float:
        ...
        # Поля которые могут содержать расстояние в ClickHouse
        for field_name in ["distance", "score", "similarity", "_distance", "_score"]:
